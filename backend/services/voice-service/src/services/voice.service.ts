...
export class VoiceService {
  ...
  async listRecordings(
    ...
    if (filter.userRole) {
      query.andWhere('recording.userRole = :userRole', {
        userRole: filter.userRole,
      });
    }

    if (filter.conversationId) {
      query.andWhere('recording.conversationId = :conversationId', {
        conversationId: filter.conversationId,
      });
    }

    if (filter.appointmentId) {
      query.andWhere('recording.appointmentId = :appointmentId', {
    ...
    if (filter.language) {
      query.andWhere('recording.language = :language', {
        language: filter.language,
      });
    }

    if (filter.startDate) {
      query.andWhere('recording.createdAt >= :startDate', {
        startDate: filter.startDate,
      });
    }

    if (filter.endDate) {
      query.andWhere('recording.createdAt <= :endDate', {
  ...
  async processRecording(recordingId: string): Promise<Transcription> {
    ...
    try {
      ...
      // Validate audio file
      const validation = await this.aiService.validateAudioFile(
        recording.fileUrl,
      );
      ...
      const medicalAnalysis = this.medicalService.analyzeMedicalContent(
        result.text,
      );
  ...
  async listTranscriptions(
    ...
    const query = this.transcriptionRepository.createQueryBuilder(
      'transcription',
    );

    if (filter.recordingId) {
      query.andWhere('transcription.recordingId = :recordingId', {
        recordingId: filter.recordingId,
      });
    }

    if (filter.userId) {
      query.andWhere('transcription.userId = :userId', {
        userId: filter.userId,
      });
    }

    if (filter.status) {
      query.andWhere('transcription.status = :status', {
        status: filter.status,
      });
    }

    if (filter.language) {
      query.andWhere('transcription.detectedLanguage = :language', {
        language: filter.language,
      });
    }

    if (filter.minConfidence !== undefined) {
      query.andWhere('transcription.confidence >= :minConfidence', {
        minConfidence: filter.minConfidence,
      });
    }

    if (filter.startDate) {
      query.andWhere('transcription.createdAt >= :startDate', {
        startDate: filter.startDate,
      });
    }

    if (filter.endDate) {
      query.andWhere('transcription.createdAt <= :endDate', {
        endDate: filter.endDate,
      });
    }
  ...
  async getStatistics(): Promise<VoiceStatistics> {
    ...
    const result = await this.transcriptionRepository
      .createQueryBuilder()
    ...
    return {
      ...
      completedRecordings,
      failedRecordings,
      avgConfidence: result?.avg ? parseFloat(result.avg) : 0,
    };
  }
}
