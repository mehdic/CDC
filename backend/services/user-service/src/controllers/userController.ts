...
export async function listUsers(req: Request, res: Response) {
  try {
    ...
    const usersResponse = users.map(user => ({
      ...
      permissions: (user as any).permissions_override || [],
...
export async function createUser(req: Request, res: Response) {
  try {
    ...
    const newUser = userRepository.create({
      ...
      permissions_override: permissions ? JSON.stringify(permissions) as any : null,
    } as any);

    const savedUser = (await userRepository.save(newUser)) as unknown as User;

    // Log audit entry
    await logAudit(currentUserId, 'user.created', 'user', savedUser.id, {
      email,
      role,
      permissions,
    }, req);
...
export async function updatePermissions(req: Request, res: Response) {
  try {
    ...
    // Update permissions
    (user as any).permissions_override = JSON.stringify(permissions);
    await userRepository.save(user);

    // Log audit entry
    await logAudit(currentUserId, 'permission.updated', 'user', user.id, {
      userId: id,
      permissions,
    }, req);
