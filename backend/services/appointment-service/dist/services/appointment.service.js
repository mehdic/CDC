...
class AppointmentService {
    constructor(appointmentRepository, availabilityRepository) {
    ...
    async createAppointment(data) {
        ...
        const hasAvailability = await this.checkAvailability(data.provider_id, data.scheduled_start);
        if (!hasAvailability) {
        ...
        const appointment = this.appointmentRepository.create({
            ...
            status: appointment_types_1.AppointmentStatus.PENDING,
            timezone: 'UTC',
        });
    ...
    async listAppointments(filters) {
        ...
        if (filters.appointment_type) {
            query = query.andWhere('appointment.appointment_type = :appointment_type', { appointment_type: filters.appointment_type });
        }
        if (filters.start_date) {
            query = query.andWhere('appointment.scheduled_start >= :start_date', { start_date: filters.start_date });
        }
        if (filters.end_date) {
    ...
    async updateAppointment(id, data) {
        ...
        if (appointment.status === appointment_types_1.AppointmentStatus.COMPLETED) {
    ...
    async confirmAppointment(id) {
        const appointment = await this.getAppointmentById(id);
        if (appointment.status !== appointment_types_1.AppointmentStatus.PENDING) {
            throw new Error('Only pending appointments can be confirmed');
        }
        appointment.status = appointment_types_1.AppointmentStatus.CONFIRMED;
    ...
    async cancelAppointment(id, reason) {
        ...
        await this.updateSlotBooking(appointment.provider_id, appointment.scheduled_start, -1);
        return appointment;
    }
    /**
    ...
    async completeAppointment(id) {
        const appointment = await this.getAppointmentById(id);
        if (appointment.status !== appointment_types_1.AppointmentStatus.CONFIRMED &&
            appointment.status !== appointment_types_1.AppointmentStatus.IN_PROGRESS) {
            throw new Error('Only confirmed or in-progress appointments can be completed');
        }
        appointment.status = appointment_types_1.AppointmentStatus.COMPLETED;
    ...
    async checkConflicts(request) {
        let query = this.appointmentRepository
            ...
            statuses: [appointment_types_1.AppointmentStatus.CANCELLED],
        })
            .andWhere('(appointment.scheduled_start < :end_time AND appointment.scheduled_end > :start_time)', {
     ...
     */
    async checkAvailability(providerId, appointmentTime) {
     ...
     */
    async updateSlotBooking(providerId, appointmentTime, increment) {
        const dayOfWeek = appointmentTime.getDay();
        const slot = await this.availabilityRepository.findOne({
            where: {
                provider_id: providerId,
     ...
     */
    async getUpcomingAppointments(providerId, daysAhead = 7) {
        const now = new Date();
        ...
        return this.appointmentRepository.find({
            where: {
                ...
                scheduled_start: (0, typeorm_1.Between)(now, futureDate),
                status: (0, typeorm_1.In)([appointment_types_1.AppointmentStatus.CONFIRMED, appointment_types_1.AppointmentStatus.PENDING]),
            },
            relations: ['patient'],
    ...
    async getAppointmentsInRange(providerId, startDate, endDate) {
        return this.appointmentRepository.find({
            where: {
                provider_id: providerId,
                deleted_at: (0, typeorm_1.IsNull)(),
                scheduled_start: (0, typeorm_1.Between)(startDate, endDate),
            },
            relations: ['patient'],
            order: { scheduled_start: 'ASC' },
        });
