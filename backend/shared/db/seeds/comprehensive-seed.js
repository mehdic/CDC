...
async function seedPharmacies(client) {
    ...
    for (const pharmacy of pharmacies) {
        const result = await client.query(`INSERT INTO pharmacies (
...
async function seedCategories(client) {
    ...
    const parentResult = await client.query(`INSERT INTO categories (name, slug, description, icon_url, parent_id, is_active, display_order)
    ...
    for (let i = 1; i < categories.length; i++) {
        ...
        const result = await client.query(`INSERT INTO categories (name, slug, description, icon_url, parent_id, is_active, display_order)
       VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id`, [category.name, category.slug, category.description, category.icon_url, parentId, true, i]);
 ...
 * Seed prescriptions
 */
async function seedPrescriptions(client, pharmacyIds, userIds) {
    ...
    for (const prescription of prescriptions) {
        const result = await client.query(`INSERT INTO prescriptions (
 ...
 */
async function seedPrescriptionItems(client, prescriptionIds) {
 ...
 */
async function seedInventoryItems(client, pharmacyIds) {
    ...
    const inventoryIds = [];
    for (const item of items) {
        const result = await client.query(`INSERT INTO inventory_items (
 ...
 */
async function seedInventoryAlerts(client, inventoryIds) {
    ...
    for (const alert of alerts) {
      ...
      ) VALUES ($1, $2, $3, $4, $5)`, [alert.inventory_item_id, alert.alert_type, alert.severity, alert.message, alert.is_resolved]);
