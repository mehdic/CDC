...
**TASK:** Fix Mobile App Security Vulnerabilities

**REQUIREMENTS:**
## Objective

Fix all security vulnerabilities in the mobile/ directory (delivery-app) flagged by Dependabot and npm audit.

## Steps

### 1. Run Security Audit
```bash
cd mobile/delivery-app && npm audit
```
...
**For remaining issues, manually update:**
- Check each vulnerable package
- Update to latest secure version
- Handle breaking changes if any

### 3. Verify No Breaking Changes
```bash
npm run build
# Or for React Native:
npx react-native bundle --platform ios --dev false --entry-file index.js --bundle-output /tmp/test.jsbundle 2>/dev/null || echo 'Bundle check skipped'
```

## Common Vulnerable Packages (Mobile)

- react-native (framework)
- metro (bundler)
- Various React Native dependencies
- Expo packages (if used)
- Native module packages

## Acceptance Criteria

- [ ] npm audit shows 0 vulnerabilities in mobile/
