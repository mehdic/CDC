...
**TASK:** Configure Sentry Error Tracking Integration

**REQUIREMENTS:**
## Objective

Configure Sentry error tracking in both web and backend applications. The @sentry/react and @sentry/node packages are already installed but NOT initialized in the source code.

## Tasks

### 1. Web App Sentry Configuration

Create web/src/config/sentry.ts:
```typescript
...
Add to web/src/main.tsx (before React render):
```typescript
...
Create backend/src/config/sentry.ts with similar pattern for Node.js.
...
- [ ] Web Sentry config created with conditional initialization
- [ ] Backend Sentry config created
- [ ] main.tsx imports and calls initSentry()
- [ ] .env.example updated with Sentry DSN placeholders
- [ ] Build succeeds
- [ ] No runtime errors when DSN is not set
