...
**TASK:** Enable react-leaflet for Delivery Tracking Map

**REQUIREMENTS:**
## Objective

Enable the react-leaflet package in the web application. The map code for delivery tracking is already implemented but react-leaflet is currently disabled/commented out in package.json.

## Tasks

1. **Enable react-leaflet in package.json**
   - Check if react-leaflet and leaflet are in dependencies
   - If commented out, uncomment them
   - If missing, add: react-leaflet@^4.2.1, leaflet@^1.9.4, @types/leaflet

2. **Verify map component works**
   - Find the delivery tracking map component
   - Ensure imports are correct (not dynamic imports if not needed)
   - Verify Leaflet CSS is imported

3. **Run build to confirm no errors**
   - npm run build should succeed
   - No TypeScript errors related to leaflet

## Location

- web/package.json
- Look for map components in web/src/apps/driver/ or delivery-related directories

## Acceptance Criteria

- [ ] react-leaflet and leaflet packages enabled in package.json
- [ ] npm install succeeds
- [ ] Build succeeds without leaflet-related errors
- [ ] Map component imports work correctly
