...
**TASK:** Create Kubernetes Production Manifests

**REQUIREMENTS:**
## Objective

Create Kubernetes manifests for production deployment of MetaPharm Connect. Deployment workflows exist in .github/workflows/ but k8s manifests are missing.
...
- Create metapharm namespace

### 2. Web Deployment (web-deployment.yaml)
- Deployment for web frontend
- Resource limits (256Mi memory, 250m CPU)
- Readiness/liveness probes
- Environment variables from ConfigMap/Secret

### 3. API Deployment (api-deployment.yaml)
...
- ClusterIP service for web (port 80)
- ClusterIP service for API (port 3000)

### 5. Ingress (ingress.yaml)
- Nginx ingress controller annotations
- TLS configuration placeholder
- Host routing for web and api subdomains

### 6. ConfigMap (configmap.yaml)
- Non-sensitive configuration
- Feature flags, API URLs

### 7. Secrets Template (secrets.yaml.template)
- Template for secrets (not actual values)
- Database credentials, API keys placeholders
...
- [ ] All manifest files created with valid YAML
