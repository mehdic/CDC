...
**SESSION:** bazinga_20251225_110636
**GROUP:** INIT
**MODE:** Parallel
**BRANCH:** main

**TASK:** Plan Implementation of Minor Gaps from Ultrathink Review

**REQUIREMENTS:**
## Context

Following the successful completion of BAZINGA session bazinga_20251215_103357, an ultrathink review identified 4 minor gaps that need to be addressed. All 55 original task groups from tasks8.md were completed with 818 passing tests, 0 failures.

## Gaps to Implement

### 1. T8-037 Patient Messaging UI Components
- **Status**: Backend messaging complete, patient app lacks messaging UI components
- **Need**: Create patient-facing messaging UI in web/src/apps/patient/
- **Reference**: Pharmacist messaging UI at web/src/apps/pharmacist/features/messaging/

### 2. Enable react-leaflet for Delivery Tracking Map
- **Status**: Map code is ready but react-leaflet is disabled in package.json
- **Need**: Enable react-leaflet package and ensure map component works
- **Location**: web/package.json, delivery tracking map component

### 3. Configure Sentry Error Tracking Integration
- **Status**: @sentry/react and @sentry/node installed but NOT configured in source
- **Need**: Initialize Sentry in web app entry point and backend
- **Note**: Create placeholder DSN config (user provides real DSN)

### 4. Create Kubernetes Production Manifests
- **Status**: Deployment workflows present, manifests may be missing or in separate repo
- **Need**: Create k8s/ directory with deployment, service, ingress manifests
- **Scope**: Web, API, and worker deployments

## Instructions

1. Analyze these 4 gaps and create task groups
2. Determine mode (simple or parallel based on dependencies)
3. Assign appropriate tier (Developer or SSE based on complexity)
4. Save task groups to database
5. Return task group assignments for Developer spawns
