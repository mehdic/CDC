...
**TASK:** Fix Unhealthy Backend Services

**REQUIREMENTS:**
Fix 7 unhealthy backend services that are preventing the application from running properly. Investigate Docker health checks, database connections, and service dependencies. The services need to start correctly and respond to health checks so the entire application can run from local database. Services to fix: analytics-service, calendar-service, controlled-substance-service, delivery-service, insurance-service, nurse-service, recycling-service.
