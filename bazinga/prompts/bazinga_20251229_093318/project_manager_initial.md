...
**TASK:** Pharmacist App Local DB Setup, Navigation Testing, and Playwright E2E

**REQUIREMENTS:**
Continue updating the app to run from local DB, fill the DB with data as necessary, then make sure every link in the pharmacist login works properly and the content from the DB is properly displayed; test all navigation links within each page with Playwright to make sure the entire app is working properly; if a section is missing development, develop it according to specifications and make sure it works. Run the entire system locally to ensure it works properly.
