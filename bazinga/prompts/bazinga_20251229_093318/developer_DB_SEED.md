...
**TASK:** Database Setup and Seeding for Pharmacist App

**REQUIREMENTS:**
Set up and seed the local PostgreSQL database with comprehensive test data for the pharmacist application. This includes: 1) Ensure all required tables exist, 2) Seed pharmacies, users with pharmacist role, prescriptions, prescription items, medications, inventory items, products, categories, etc. 3) Ensure foreign key relationships are maintained, 4) Use Swiss/French names for realistic data, 5) The data should support all pharmacist app features: prescription processing, inventory management, patient records, etc.
