...
**TASK:** Fix: Nurse E2E Tests - Port Config and API Mocks

**REQUIREMENTS:**
TECH LEAD UNBLOCKING GUIDANCE:

ISSUE: E2E tests blocked because playwright.config.ts targets port 3000 (BAZINGA Dashboard) instead of port 5173 (MetaPharm Connect app).

FIXES REQUIRED:
1. Update e2e/playwright.config.ts:
   - Change baseURL from localhost:3000 to localhost:5173
   - Change webServer.url from localhost:3000 to localhost:5173

2. Add API mocks for login flow:
   - Nurse E2E tests cannot locate login form elements
   - Add proper mock responses for authentication endpoints
   - Ensure login page renders correctly in E2E environment

After fixes, the 342 Nurse E2E tests should be unblocked and able to execute.
...
Port mismatch: playwright.config.ts targets wrong port (3000 vs 5173). Fix port config first, then add API mocks for login flow.
