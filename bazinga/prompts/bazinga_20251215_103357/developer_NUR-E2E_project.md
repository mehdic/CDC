...
**TASK:** Fix NUR-E2E project filter and page rendering

**REQUIREMENTS:**
QA FAIL: 0/110 tests passed. Tests ran on all 10 Playwright projects instead of just nurse project. Page elements not loading.

ISSUES TO FIX:
1. Add project filter to run only nurse-related tests (not all 10 projects)
2. Fix NurseDashboard component rendering - page elements not loading in tests

FILES TO CHECK:
- web/e2e/playwright.config.ts (project configuration)
- web/src/apps/nurse/pages/NurseDashboard.tsx (component rendering)
- web/e2e/tests/nurse-login-dashboard.spec.ts (test setup)

Test command: MOCK_AUTH=true npm run test:e2e -- --project=nurse nurse-login-dashboard.spec.ts
...
0/110 tests - tests ran on all 10 projects instead of nurse only. Page elements not loading. Add project filter and fix NurseDashboard rendering.
