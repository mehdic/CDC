...
**TASK:** T8-050: Security E2E Tests - Add Missing TestIDs

**REQUIREMENTS:**
QA Expert reported 30/594 tests passing (5%).

The security TESTS are written but many test against features not yet implemented. Focus on:
1. Add missing data-testid attributes to existing security UI components
2. Skip tests for unimplemented features (CSRF, rate limiting, account lockout)

DO NOT implement all missing security features - just fix testids and mark unimplemented tests as skipped.
...
30/594 tests passed (5.1%). Missing implementation: CSRF protection, security headers, rate limiting, account lockout, session regeneration, password reset UI, and data-testid attributes. Focus on testids and skip unimplemented feature tests.
