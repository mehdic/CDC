...
**[HIGH]** bazinga/artifacts/bazinga_20251215_103357/E2E-A11Y/qa_failures_E2E-A11Y_r4.md
> 101 WCAG accessibility test failures: 45 keyboard navigation, 9 dashboard violations (260+ sub-violations), 9 color contrast, 9 order tracking, 2 mobile patient list

**[HIGH]** bazinga/artifacts/bazinga_20251215_103357/failures_E2E-A11Y_r3.md
> 92 E2E test failures - API mocking still broken after Round 3, developer fixed WCAG but ignored infrastructure

**[HIGH]** bazinga/artifacts/bazinga_20251215_103357/E2E-A11Y/qa_failures_E2E-A11Y.md
> 67 E2E accessibility test failures due to routing and nurse API mocking issues
...
1. **Fixed Root Cause: Nested Main Elements (WCAG landmark-unique violation)**
   - Identified that AppShell.tsx and NurseLayout.tsx provide `<main>` elements via styled components
   - Dashboard components were adding NESTED `component="main" role="main"` -

**[senior_software_engineer] approach:** ## Root Cause Analysis

### The Mystery Solved
Developer added `role="main"` to dashboards, but tests kept failing. Why?

### Problem: Nested Main Elements

**The architecture creates MULTIPLE main elements:**

1. **AppShell (line 47-67):**
   ```tsx
   const Main = styled('main', ...)  // Creates <

**[senior_software_engineer] understanding:** ## Understanding

### Task Interpretation
Escalated SSE task: Developer fixed WCAG accessibility violations 5 times, but QA keeps finding the same 101 failures (85.2% pass rate). The fixes exist in source code but are not detected by E2E tests. This is an infrastructure/configuration issue, not a co
...
**CRITICAL DISCOVERY:** Developer's fixes were applied but had ZERO impact on test results.
- Round 4: 583/684 passing (101 failures)
- Round 5: 583/684

**[qa_expert] understanding:** ## Understanding

### Test Scope
Round 5 of E2E Accessibility testing after developer fixed 3 HIGH priority WCAG violations:
1. Added role="main" to all 5 dashboards
2. Added aria-label to progress bars (CircularProgress components)
3. Fixed route definitions
4. Created PatientDashboard component
...
**TASK:** T8-052: Accessibility E2E Tests - SSE Fix Review

**REQUIREMENTS:**
SSE found and fixed root cause.

Root cause: Nested <main> landmarks in layout/dashboard architecture causing WCAG landmark-unique violations.

Fixes applied:
- Modified 13 files (AppShell, NurseLayout, 5 dashboards, 6 nurse components)
- Added aria-labels to IconButtons and CircularProgress
- Test pass rate improved 85.2% → 91.2% (624/684)

Remaining 60 failures are keyboard navigation test expectations, not WCAG violations.

Assess if 91.2% is acceptable or if more work needed.
