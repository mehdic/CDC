...
**TASK:** Fix NUR-E2E storage state and fixture auth

**REQUIREMENTS:**
TECH LEAD ROOT CAUSE IDENTIFIED - Two cascading infrastructure issues:

1. ISSUE 1 (Critical): Storage state key mismatch
   - global-setup.ts saves auth with key 'user'
   - authService.ts reads 'user_data'
   - FIX: Change global-setup.ts to use 'user_data' key

2. ISSUE 2: Fixture auth method wrong
   - nursePage fixture uses mockLoginSuccess() instead of loginWithStoredAuth()
   - FIX: Update nursePage fixture to use loginWithStoredAuth() method

FILES TO FIX:
- web/e2e/config/global-setup.ts (change localStorage key from 'user' to 'user_data')
- web/e2e/fixtures/nursePage.ts (use loginWithStoredAuth instead of mockLoginSuccess)

Test: MOCK_AUTH=true npm run test:e2e -- --project=nurse nurse-login-dashboard.spec.ts
...
Root cause: storage state key mismatch (user vs user_data), fixture auth method wrong. Apply 2 specific fixes.
