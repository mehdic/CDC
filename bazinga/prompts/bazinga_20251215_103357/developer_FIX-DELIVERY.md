...
**TASK:** Fix Delivery Component Test Failures

**REQUIREMENTS:**
Fix failing tests in web/src/apps/driver/.

FAILING FILES:
1. DeliveryCard.test.tsx - Text mismatch (expects 'In Transit' but gets 'IN_TRANSIT')
2. DeliveryListScreen.test.tsx - Element not found (getByText failing)
3. DeliveryFilters.test.tsx - Multiple elements with same text
4. DeliveryDetailScreen.test.tsx - waitFor timeout on element query

FIX STRATEGIES:
- Use getByTestId instead of getByText for ambiguous elements
- Fix text case: ensure component displays 'In Transit' not 'IN_TRANSIT'
- Add data-testid attributes where needed
- Increase waitFor timeout or fix async issues

Run: npm test -- --testPathPattern='driver' --passWithNoTests --watchAll=false
Target: 0 failures in driver tests.
