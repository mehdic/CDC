...
**TASK:** T8-026: Offline Mode & Sync Queue
...
- Queue actions when offline (proof of delivery, status updates)
- Sync when connection restored (automatic)
- Conflict resolution (server-wins with notification)
- Local data persistence (AsyncStorage or MMKV)

## Implementation Location
- mobile/delivery-app/src/services/offlineQueueService.ts
- mobile/delivery-app/src/hooks/useOfflineSync.ts
- mobile/delivery-app/src/components/OfflineIndicator.tsx
- mobile/delivery-app/src/store/offlineSlice.ts

## Acceptance Criteria
- [ ] Actions queued offline (POD submissions, status updates)
- [ ] Automatic sync on reconnect (within 30 seconds)
- [ ] Conflicts handled (server-wins with user notification)
...
- Queue structure: { id, action, payload, timestamp, retryCount }
