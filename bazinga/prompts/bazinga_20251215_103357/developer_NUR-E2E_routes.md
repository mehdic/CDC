...
**TASK:** Implement Nurse Routes in AppRoutes

**REQUIREMENTS:**
TECH LEAD CHANGES_REQUESTED:

ISSUE: E2E tests expect /nurse/* routes but these don't exist in web/src/routes/index.tsx

NurseLayout.tsx and NurseDashboard.tsx already navigate to /nurse/* routes, but the main router doesn't register them.

IMPLEMENTATION REQUIRED:

1. Add lazy imports in web/src/routes/index.tsx:
   - NurseLayout from '@apps/nurse/layouts/NurseLayout'
   - NurseDashboard from '@apps/nurse/components/NurseDashboard'
   - PatientList from '@apps/nurse/components/PatientList' (if exists)
   - MedicationOrderForm from '@apps/nurse/components/MedicationOrderForm' (if exists)
   - OrderTracking from '@apps/nurse/components/OrderTracking' (if exists)

2. Add /nurse/* routes (before 404 route):
   - /nurse parent route with NurseLayout and ProtectedRoute (requiredRoles=['nurse', 'admin'])
   - /nurse (index) -> NurseDashboard
   - /nurse/patients -> PatientList
   - /nurse/orders -> MedicationOrderForm
   - /nurse/tracking -> OrderTracking
   - /nurse/notifications -> NotificationPanel

3. NurseLayout uses <Outlet /> for nested routing (line 303) - follow this pattern.

4. Check if test paths match NavItems (/nurse/medications vs /nurse/orders) - align as needed.

REFERENCE: bazinga/artifacts/bazinga_20251215_103357/NUR-E2E/review_details.md contains code examples.
...
CHANGES_REQUESTED: Implement /nurse/* routes in AppRoutes. NurseLayout already supports nested routing via Outlet. See review_details.md for code examples.
