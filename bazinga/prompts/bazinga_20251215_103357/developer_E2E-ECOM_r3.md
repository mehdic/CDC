...
**TASK:** T8-049: E-Commerce E2E Tests - Fix PATIENT Components

**REQUIREMENTS:**
QA Expert Round 2 reported 206/234 tests passing (88%), 28 failures.

ROOT CAUSE: Developer added testids to PHARMACIST components but tests run as PATIENT role.

Fix required:
1. Find the PATIENT e-commerce components (not pharmacist)
2. Add data-testid="add-to-cart-{productId}" to patient's add-to-cart buttons
3. Add data-testid="cart-icon" to patient's cart icon
4. Add data-testid="orders-list" to patient's orders list

The /ecommerce route may load pharmacist components - verify which components are used for PATIENT role.
...
206/234 E2E tests passed (88%), 28 failures. Developer's fix did NOT work - same 3 core issues persist (add-to-cart, cart-icon, orders-list missing data-testid). Developer likely added attributes to WRONG components (pharmacist instead of patient).
