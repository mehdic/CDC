...
**TASK:** Fix Nurse Dashboard Test Failures

**REQUIREMENTS:**
Fix failing tests in web/src/apps/nurse/.

FAILING FILE:
1. NurseDashboard.test.tsx - React Router deprecation warnings causing failures

FIX STRATEGIES:
- Update React Router usage to v6 patterns (remove deprecated APIs)
- Use MemoryRouter correctly in tests
- Suppress or fix UNSAFE_logV6DeprecationWarnings
- Ensure test passes without React Router warnings

Run: npm test -- --testPathPattern='nurse' --passWithNoTests --watchAll=false
Target: 0 failures in nurse tests.
