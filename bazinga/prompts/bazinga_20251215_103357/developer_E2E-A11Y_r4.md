...
**TASK:** T8-052: Accessibility E2E Tests - Fix Dashboard API Mocking

**REQUIREMENTS:**
QA Expert Round 3 found 592/684 tests passing but dashboards show 'Error loading dashboard'.

The WCAG fixes from r3 are correct, but dashboard API mocking is broken.

Fix required:
1. Fix API mocking for dashboard endpoints
2. Ensure all role dashboards load properly in E2E tests
3. Use existing MOCK_AUTH pattern from other E2E tests

This is an infrastructure/mocking issue, not WCAG violations.
...
592/684 E2E tests passed, 92 failed. Developer fixed WCAG violations but ignored dashboard API mocking infrastructure. All dashboards show 'Error loading dashboard' - tests cannot run until API mocks fixed.
