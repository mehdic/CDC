...
**TASK:** Fix Patient Feature Test Failures

**REQUIREMENTS:**
Fix failing tests in web/src/apps/patient/.

FAILING FILES:
1. DigitalTwinDashboard.test.tsx - Network error: 'Error fetching profile'
2. Catalog.integration.test.tsx - Component test issues
3. Catalog.e2e.test.tsx - Component test issues

FIX STRATEGIES:
- Add proper network mocks for API calls
- Mock useDigitalTwin hook to prevent network errors
- Fix component rendering in test environment
- Ensure all API calls are properly mocked

Run: npm test -- --testPathPattern='patient' --passWithNoTests --watchAll=false
Target: 0 failures in patient tests.
